...
runAnalysis <- function(meta){
  ...
  mouse_geneID <- read.delim(sprintf("%s/mm10_vM18.genes.cleaned.bed",indir),header=T)  
  mat <- mat[rownames(mat) %in% mouse_geneID$gene_id,]
  mat <- mat[!duplicated(rownames(mat)),]
  
  mm10_geneID <- read.delim("../../../03.data/01.ref/mm10.genes.bed",header=T)  
  ...
  rownames(mat) <- mm10$gene_id_long[match(rownames(mat),mm10$gene_id_short)]
