...
for(mc in mc_type){
    for(dt in datatype){
        our_correlation<-read.csv(paste0("../../../output/01-youth/02-correlation_calculation/all_cell_correlation/all_cells_",dt,"_",mc,"_",var,"_gene_correlation_results.csv"))
        colnames(our_correlation)[1]<-"gene_id"
        data<-our_correlation
        for(i in 1:100){
            cat(paste0(dt,"-",mc,"-",i),'\n')
        ...
        write.csv(data,paste0("../../../output/01-youth/02-correlation_calculation/shuffled_correlation/shuffled_",dt,"_",mc,"_",var,"_gene_correlation_total_results.csv"),row.names=F)
    }
}
