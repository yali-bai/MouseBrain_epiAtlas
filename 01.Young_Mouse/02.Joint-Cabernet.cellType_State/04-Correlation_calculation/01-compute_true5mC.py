...
def compute_5mC(var_dim,mc_type):  
    #Load two h5ad files
    hmC = sc.read_h5ad(f'{indir}/5hmC_{var_dim}.{mc_type}.pass_paired_QC.h5ad')  
    mC = sc.read_h5ad(f'{indir}/5mC_{var_dim}.{mc_type}.pass_paired_QC.h5ad')  
    ...
    info=pd.read_csv("../../../input/01-youth/RNA_DNA_match_name_QC_class_label.csv")
    info.index='allc_'+info['hmC']
    hmC.obs_names=info['Unique_ID_match'][hmC.obs_names]
    info.index='allc_'+info['mC']
    mC.obs_names=info['Unique_ID_match'][mC.obs_names]    
    mC=mC[hmC.obs_names,:]
    print(hmC.obs_names.equals(mC.obs_names))
    #compute true5mC
    ...
    #add obs and var
    columns_to_add = ['SampleID', 'Plate_ID', 'Sample_ID_without_barcode', 'Unique_ID', 'Species', 'Datatype', 'Library', 'Region', 'Brain.Region', 'Batch', 'spatial_type', 'Age','class_label', 'subclass_label', 'Neuron_non_neuron', 'three_class_label']  
    selected_columns = hmC.obs[columns_to_add]
    true5mC_adata.obs[columns_to_add] = selected_columns  
    true5mC_adata.obs['Library']='true-5mC'
    true5mC_adata.var = hmC.var
    true5mC_adata.write_h5ad(f'{outdir}/true_5mC_{var_dim}.{mc_type}.pass_paired_QC.h5ad')
