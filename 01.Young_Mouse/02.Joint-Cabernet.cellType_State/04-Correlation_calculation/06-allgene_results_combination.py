...
for mc_type in mc_types:
    for datatype in datatypes:
        for var_dim in var_dims:
            ...
            data=pd.read_csv(f"../../../output/01.Young_Mouse/02-correlation_calculation/shuffled_correlation/shuffled_{datatype}_{mc_type}_{var_dim}_gene_correlation_total_results.csv")
            df_filtered = data.dropna(subset=['Correlation'])
            df=df.append(df_filtered) 

#add RNA mean /RNA cv
RNA_metainfo=pd.read_csv("../../../output/01.Young_Mouse/02-correlation_calculation/RNA_metainfo_data.csv")
merged_df = pd.merge(df, RNA_metainfo, on='gene_id') 
#add gene length/cpg number
gene_metainfo=pd.read_csv("../../../output/01.Young_Mouse/02-correlation_calculation/gene_metainfo.csv")
...
merged_df2.to_csv('../../../output/01.Young_Mouse/02-correlation_calculation/shuffled_RNA_DNA_correlation_result_all.csv', index=False)               
