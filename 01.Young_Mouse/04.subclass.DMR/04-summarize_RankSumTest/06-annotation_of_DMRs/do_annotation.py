...
def DMR_states_h5ad_to_region_ds(h5ad_path, output_dir):
    ...
    subprocess.run(f"cp chrom_sizes.txt {output_dir}/chrom_sizes.txt", shell=True)
    return


list_h5ad_path=list(pathlib.Path(f"{indir}/").glob(pattern="*/*DMR_states*.h5ad"))

list_output_path=[f"{outdir}/RegionDS_data/"+str(i).split("/")[-1].split(".")[0] for i in list_h5ad_path]
...
list_RegionDS=list(pathlib.Path(outdir).glob("RegionDS_data/*DMR_states*"))

for i in tqdm(list_RegionDS):
    dmr_ds = RegionDS.open(i, region_dim='dmr')
    dmr_ds.annotate_by_beds(slop=250,
                        bed_table=f'{outdir}/new_genome_featue_bed.csv',
                        dim='genome-features',
                        bed_sorted=False,
                        cpu=38)
