...
modification, state, region = sys.argv[1:4]
...
df_RNA=pd.read_csv('../../03.data/04.config_files/02-our_subclass_mean_dat_final.csv',index_col=0).T
...
DMR_states_df=ad.read_h5ad(f"../03-stratified_RankSumTest_1vsOthers/01_subclasses_in_neuron_1vsOthers/{modification}_DMR_states_neuron_1vsOthers.h5ad").to_df()
...
if region == "promoter":
    genes_df = pd.read_csv('../../../03.data/01.ref/PromoterUp2k.mm10.bed', sep='\t', header=None, names=['chromosome', 'start', 'end', 'gene_id', 'gene_name', 'strand'])
else:
...
df_DMR_geneid_diff.to_csv(f"{outdir}/{"_".join([modification, state, region])}-DMR_GeneID_list.csv", index=False)
...
for modification_show in ["5hmCG", "total5mCG", "true5mCG"]:
    ...
    plt.savefig(f"{outdir}/DMR_plot-{"_".join([modification, state, region])}-{modification_show}_levels.pdf")
    ...
    plt.ylabel(f"{state}-{"DHMR" if modification=="5hmCG" else "DMR"}s overlaping {"genebodies" if region =="genebody" else "promoters"}",fontdict={"size":font_size}, labelpad=2)
    plt.title(f"{"5hmCG" if modification_show=="5hmCG" else ("5mCG+5hmCG" if modification_show=="total5mCG" else "5mCG")} levels",fontdict={"size":font_size},pad=3)
    plt.savefig(f"{outdir}/DMR_plot-{"_".join([modification, state, region])}-{modification_show}_levels.png", dpi=600, bbox_inches="tight")
...
plt.savefig(f"{outdir}/RNA_plot_{"_".join([modification, state, region])}.pdf")
...
plt.ylabel(f"{state}-{"DHMR" if modification=="5hmCG" else "DMR"}s overlaping {"genebodies" if region =="genebody" else "promoters"}",fontdict={"size":font_size}, labelpad=2)
plt.title("normalized RNA expression",fontdict={"size":font_size},pad=3)
plt.savefig(f"{outdir}/RNA_plot_{"_".join([modification, state, region])}.png",dpi=600, bbox_inches="tight")
