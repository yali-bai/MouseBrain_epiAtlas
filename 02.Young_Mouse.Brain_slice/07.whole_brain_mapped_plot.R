...
#spatial_zhuang_merfish_loci.df = read.csv("/share/analysisdata/Methyl/public/analysis/data/MERFISH/Zhuang_dataset/cell_metadata_Zhuang_MERFISH.csv",colClasses = c("character","character","character","character","character","character","character","numeric","numeric","numeric","numeric","numeric","character"))
spatial_zhuang_merfish_loci.df = read.csv("../input/02-slice/cell_metadata_Zhuang_MERFISH.csv",colClasses = c("character","character","character","character","character","character","character","numeric","numeric","numeric","numeric","numeric","character"))
...
merged.seuratobj.sct = readRDS("../input/02-slice/map/merged.seuratobj.sct.loci_transfer.the_nearst_1_cell.rds")
merged.seuratobj.sct_subset = subset(merged.seuratobj.sct,three_class == "Exc") # subset by three class information 
plot_df = data.frame(Embeddings(merged.seuratobj.sct_subset, reduction = 'umap')[rownames(merged.seuratobj.sct_subset@meta.data)[which(merged.seuratobj.sct_subset$source == "Our")],])
...
pdf("../output/02-slice/map/Exc_spatial.whole_biran.pdf",,width = 40,height = 50)
ggplot() +
...
pdf("../output/02-slice/map/Inh_spatial.whole_biran.pdf",,width = 40,height = 50)
ggplot() +
            geom_point(data = spatial_zhuang_merfish_loci.df, mapping = aes(umap_1, umap_2), size = 3, color = "lightgrey") + # size = 15
            geom_point(data = plot_df, mapping = aes(umap_1, umap_2, color=subclass), size = 12) +
            scale_color_manual(values=inte.col)+
...
pdf("../output/02-slice/map/Non_neuron_spatial.whole_biran.pdf",,width = 40,height = 50)
ggplot() +
            geom_point(data = spatial_zhuang_merfish_loci.df, mapping = aes(umap_1, umap_2), size = 3, color = "lightgrey") + # size = 15
            geom_point(data = plot_df, mapping = aes(umap_1, umap_2, color=subclass), size = 12) +
            scale_color_manual(values=inte.col)+
