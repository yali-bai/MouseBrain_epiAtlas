...
spatial_zhuang_merfish_loci.df = read.csv("../03.data/03.download_data/cell_metadata_Zhuang_MERFISH.csv",colClasses = c("character","character","character","character","character","character","character","numeric","numeric","numeric","numeric","numeric","character"))
...
unique(merged.seuratobj.sct$three_class)
...
pdf("07.whole_brain_mapped_plot/Exc_spatial.whole_biran.pdf",width = 40,height = 50)
ggplot() +
...
plot_df = data.frame(Embeddings(merged.seuratobj.sct_subset, reduction = 'umap')[rownames(merged.seuratobj.sct_subset@meta.data)[which(merged.seuratobj.sct_subset$source == "Joint_Cabernet")],])
plot_df$subclass = merged.seuratobj.sct@meta.data[rownames(plot_df),"subclass"]
pdf("07.whole_brain_mapped_plot/Inh_spatial.whole_biran.pdf",width = 40,height = 50)
ggplot() +
            geom_point(data = spatial_zhuang_merfish_loci.df, mapping = aes(-umap_1, -umap_2), size = 3, color = "lightgrey") + # size = 15
            geom_point(data = plot_df, mapping = aes(-umap_1, -umap_2, color=subclass), size = 12) +
            scale_color_manual(values=inte.col)+
...
merged.seuratobj.sct_subset = subset(merged.seuratobj.sct,three_class == "Non_neuron")
plot_df = data.frame(Embeddings(merged.seuratobj.sct_subset, reduction = 'umap')[rownames(merged.seuratobj.sct_subset@meta.data)[which(merged.seuratobj.sct_subset$source == "Joint_Cabernet")],])
plot_df$subclass = merged.seuratobj.sct@meta.data[rownames(plot_df),"subclass"]
pdf("07.whole_brain_mapped_plot/Non_neuron_spatial.whole_biran.pdf",width = 40,height = 50)
ggplot() +
            geom_point(data = spatial_zhuang_merfish_loci.df, mapping = aes(-umap_1, -umap_2), size = 3, color = "lightgrey") + # size = 15
            geom_point(data = plot_df, mapping = aes(-umap_1, -umap_2, color=subclass), size = 12) +
            scale_color_manual(values=inte.col)+
