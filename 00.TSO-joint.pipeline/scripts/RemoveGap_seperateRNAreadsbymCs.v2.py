...
parser = argparse.ArgumentParser(
    description="The code to remove the gap filled region by Tn5 and seperate the RNA reads by mCs. Created in 2023.10.17 by Bailey. Updated for apply_async by Liuhao Ren on 20240813.",
...
parser.add_argument("--min_CH_num",
                        type=int,
                        default = 3,
                        help="The min CH number in one fragment, Default is 3")
parser.add_argument("--min_mCH_ratio",
                        type=int,
                        default = 0.5,
                        help="The min mCH ratio in one fragment, Default is 0.5")
parser.add_argument("--max_mCH_ratio",
                        type=int,
                        default = 0.5,
                        help="The max mCH ratio in one fragment, Default is 0.5")
...
min_mCH_ratio = args.min_mCH_ratio
max_mCH_ratio = args.max_mCH_ratio
...
def trim_read (read, chrom, clip_5_end, clip_3_end, trim_t, fragment_5_end, fragment_3_end, MapQ_flag, CH_flag):
...
def PE_process(samfile,chrom, out_rna, out_dna, out_puzz):
    global lock
    ...
    out_rna = pysam.AlignmentFile(out_rna, "wb", header=header)
    out_dna = pysam.AlignmentFile(out_dna, "wb", header=header)
    out_puzz = pysam.AlignmentFile(out_puzz, "wb", header=header)
    ...
    for read1, read2 in read_pair_generator(samfile,chrom):
        ...
        if fragment_3_end - fragment_5_end > Gap_len + Gap_len:
            ...
            if read1_XG_tag == 'CT' and read2_XG_tag == 'CT':
                ...
                Gap_pre_len = 0
                Gap_off_len = Gap_len
                Read1_XM = read1_XM_tag[:-Gap_len]
                Read2_XM = read2_XM_tag[:-Gap_len]
            ...
            read1_mCHs_n = Read1_XM.count('X') + Read1_XM.count('H')
            read2_mCHs_n = Read2_XM.count('X') + Read2_XM.count('H')
            read1_CHs_n = Read1_XM.count('x') + Read1_XM.count('h')
            read2_CHs_n = Read2_XM.count('x') + Read2_XM.count('h')
            read1_mCGs_n = Read1_XM.count('Z')
            read2_mCGs_n = Read2_XM.count('Z')
            read1_CGs_n = Read1_XM.count('z')
            read2_CGs_n = Read2_XM.count('z')

            CH_num = read1_CHs_n + read2_CHs_n + read1_mCHs_n + read2_mCHs_n
            CG_num = read1_CGs_n + read2_CGs_n + read1_mCGs_n + read2_mCGs_n
            mCH_num = read1_mCHs_n + read2_mCHs_n
            mCG_num = read1_mCGs_n + read2_mCGs_n


            if (CH_num >0 and CG_num >0):
                mCH_ratio = 1.0 * mCH_num / CH_num
                mCG_ratio = 1.0 * mCG_num / CG_num
                mC_ratio = 1.0 * (mCH_num + mCG_num) / (CH_num + CG_num)
            elif (CG_num >0):
                mCH_ratio = 0
                mCG_ratio = 1.0 * mCG_num / CG_num
                mC_ratio = mCG_ratio
                ...
                mCH_ratio = mCG_ratio = mC_ratio = 0

            CH_flag = "CHn"+str(CH_num)+"_mCHn"+str(mCH_num)+"_CGn"+str(CG_num)+"_mCGn"+str(mCG_num)

            r1_new = trim_read (read1, chrom, r1_pre_tim, r1_off_tim, trim_t, fragment_5_end, fragment_3_end, MapQ_flag, CH_flag)
            r2_new = trim_read (read2, chrom, r2_pre_tim, r2_off_tim, trim_t, fragment_5_end, fragment_3_end, MapQ_flag, CH_flag)
            r1_new.next_reference_start = r2_new.reference_start
            r2_new.next_reference_start = r1_new.reference_start


            if(CH_num >= min_CH_num and mCH_ratio >= min_mCH_ratio):
                lock.acquire()
                out_rna.write(r1_new)
                out_rna.write(r2_new)
                lock.release()
            elif(CH_num >= min_CH_num and mCH_ratio < max_mCH_ratio):
                lock.acquire()
                out_dna.write(r1_new)
                out_dna.write(r2_new)
                lock.release()
            else:
                lock.acquire()
                out_puzz.write(r1_new)
                out_puzz.write(r2_new)
                lock.release()


#           print "CH_num, mCH_num, mCH_ratio, CG_num, mCG_num, mCG_ratio, CH_num + CG_num, mCH_num + mCG_num, mC_ratio"
            #print CH_num, mCH_num, mCH_ratio, CG_num, mCG_num, mCG_ratio, CH_num + CG_num, mCH_num + mCG_num, mC_ratio, Read1_XM, Read2_XM

            #if (mCH_ratio < 0.9 and mC_ratio < 0.5):
...
def SE_process(samfile,chrom, out_rna, out_dna, out_puzz):
    global lock
    lock = multiprocess.Lock()
    ...
    for read1 in samfile.fetch(chrom):
      ...
      #      print r1_pre_tim, r1_off_tim
            ...
            CH_num = read1_CHs_n + read1_mCHs_n
            CG_num = read1_CGs_n + read1_mCGs_n
            mCH_num = read1_mCHs_n
            mCG_num = read1_mCGs_n
...
def run_all():
    ...
    while Chrs:
        ...
        for chrom in Chrs[:]:
            ...
            if chrom == "chrM":
                ...
                continue
            print chrom
            rna_out_tmp = output_bam + '.' + chrom + ".rna.bam"
            dna_out_tmp = output_bam + '.' + chrom + ".dna.bam"
            puzz_out_tmp = output_bam + '.' + chrom + ".puzz.bam"
            tmp_RnaOut.add(rna_out_tmp)
            tmp_DnaOut.add(dna_out_tmp)
            tmp_PuzzOut.add(puzz_out_tmp)
            if Read_type == 'PE':
                result_dict[chrom] = pool.apply_async(LogExceptions(PE_process), (input_bam, chrom, rna_out_tmp, dna_out_tmp, puzz_out_tmp, ))
            #    PE_process(input_bam, chrom, file_out_tmp)
            else:
                result_dict[chrom] = pool.apply_async(LogExceptions(SE_process), (input_bam, chrom, rna_out_tmp, dna_out_tmp, puzz_out_tmp, ))
    ...
    out_pre = re.sub(".bam$", "", output_bam)
    print out_pre

    Args_m = ['samtools merge -f -@ %s %s.rna.bam %s' % (threads, out_pre, " ".join(tmp_RnaOut))]
    subprocess.check_call(Args_m, shell=True)
    Args_m = ['samtools merge -f -@ %s %s.dna.bam %s' % (threads, out_pre, " ".join(tmp_DnaOut))]
    subprocess.check_call(Args_m, shell=True)
    Args_m = ['samtools merge -f -@ %s %s.puzz.bam %s' % (threads, out_pre, " ".join(tmp_PuzzOut))]
    subprocess.check_call(Args_m, shell=True)


    for my_file in tmp_RnaOut:
        if os.path.exists(my_file):
            os.remove(my_file)
    for my_file in tmp_DnaOut:
        if os.path.exists(my_file):
            os.remove(my_file)
    for my_file in tmp_PuzzOut:
        if os.path.exists(my_file):
            os.remove(my_file)
    t2=clock()
