...
def run_all():
    ...
    while Chrs:
        ...
        for chrom in Chrs[:]:
            ...
            tmp_ReadOut.add(file_out_tmp)
            if Read_type == 'PE':
                result_dict[chrom] = pool.apply_async(LogExceptions(PE_process), (input_bam, chrom, file_out_tmp, ))
            #    PE_process(input_bam, chrom, file_out_tmp)
            else:
                result_dict[chrom] = pool.apply_async(LogExceptions(SE_process), (input_bam, chrom, file_out_tmp, ))
